...
class TriggerManager implements Types.TriggerManagerInterface {
	...
	private groupedTriggers: { [key: string]: Trigger[] } = {};
	...
	public remove(id: Types.TriggerID) {
	...
	public async eventHandler(event: Types.Event, args: Types.EventArgs, database: Database) {
		...
		if (this.groupedTriggers.hasOwnProperty(event.toString())) {
			const sessions: Types.TriggerSession[] = this.groupedTriggers[event.toString()].map(trig => uniqid(`${trig.id}-`));
